...
- [ ] `src/app/(app)/layout.tsx` 작성 — 세션+프로필 실제 조회, 인가 게이트
- [ ] 상단 네비게이션 바 컴포넌트 (투표/식당/랭킹/로그아웃 메뉴)
- [ ] 모바일 반응형 네비게이션 (햄버거 메뉴)
- [ ] `src/app/layout.tsx` 한글 브랜딩/메타데이터로 교체 (현재 create-next-app 보일러플레이트)
- [ ] `src/app/page.tsx` — 세션 있으면 `/votes`로 리다이렉트, 없으면 랜딩 페이지로 교체
- [ ] 랜딩 페이지 카피/디자인 (서비스 소개, 로그인/가입 CTA)
- [ ] 공통 로딩 스피너/스켈레톤 컴포넌트
- [ ] 공통 `error.tsx` 작성
- [ ] 공통 `not-found.tsx` 작성
- [ ] 공통 버튼/입력/카드 UI 컴포넌트 정리 (재사용 가능하게)
- [ ] 다크모드 대응 여부 결정 및 적용
